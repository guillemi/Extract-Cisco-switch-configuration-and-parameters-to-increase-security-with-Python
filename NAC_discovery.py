...
def to_json(ip,version,model,interface,vlan,dot1x,auto,country,typ,os,ping,con,siteID,bcc):
    ...
    up_to_json(siteID,d_data)





def read_data(ip,version,model,interface,vlan,dot1x,aut,country,typ,os,ping,con,siteID,bcc):
    
    to_json(ip,version,model,interface,vlan,dot1x,aut,country,typ,os,ping,con,siteID,bcc)
...
def run(ip,net_connect,country,typ,ping,con,siteID):
            ...
            for a in range(len(interface_config_list)):
                ...
                for b in range(len(vlan_list)):
                    ...
                    if(interface_config_list[a] == " switchport access vlan " + vlan_list[b]):
                        ...
                        read_data(ip,version,model,current[1],vlan_list[b],auth_control,autentication,country,typ,"IOS",ping,con,siteID,bcc)
                        print("match")
                    


    
def catch_device_info(current_ip_address,country,typ,siteID):
    ...
    if not ping_result:
        write_result(current_ip_address)
        read_data(current_ip_address,"-","-","-","-","-","-","-","-","-",ping,False,siteID,"-")
    else:
        ping = True
        ...
        try:
            try:
                ...
                run(current_ip_address,net_connect,country,typ,ping,con,siteID)
            except:
            ...
            con = False
            read_data(current_ip_address,"-","-","-","-","-","-","-","-","-",ping,con,siteID,"-")
